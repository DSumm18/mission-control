...
export default function DecisionsPage() {
  ...
  const [decidingBoard, setDecidingBoard] = useState<string | null>(null);
  ...
  async function recordDecision(boardId: string) {
    if (!selectedOption) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/ed/challenge", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        toast(d.error || "Failed to record decision", "bad");
        return;
      }
      toast(`Decision recorded: Option ${selectedOption}`, "good");
      setDecidingBoard(null);
      setSelectedOption("");
      setRationale("");
      loadBoards();
    } finally {
  ...
  const openBoards = boards.filter(
    (b) => b.status === "open" || b.status === "deliberating",
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <PageInfo
          title="Decisions"
          description="Challenge board history. See strategic decisions made by the AI team — options considered, pros/cons, and final outcomes."
          features={[
            "Each decision shows all options with supporting agents",
      ...
      {tasks.length > 0 && (
        ...
        >
          ...
          <div className="muted" style={{ marginTop: 8, fontSize: 11 }}>
            Tell Ed &quot;approve [task name]&quot; to sign off via voice or
            text.
          </div>
        </div>
      )}
        ...
        {(["all", "open", "decided"] as const).map((f) => (
          ...
          >
            {f === "all"
              ? `All (${boards.length})`
              : f === "open"
                ? `Open (${openBoards.length})`
                : `Decided (${decidedBoards.length})`}
          </button>
        ))}
      </div>
        ...
        {filtered.map((board) => {
          ...
          return (
            ...
            >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 600, fontSize: 14 }}>
                      {board.decision_title}
                    </span>
                    <span
                      className={`badge ${statusColor(board.status)}`}
                      style={{ fontSize: 10 }}
                    >
                      {board.status}
                    </span>
                  ...
                  {board.decision_context && (
                    <div
                      className="muted"
                      style={{ fontSize: 12, marginLeft: 28, lineHeight: 1.4 }}
                    >
                      {board.decision_context}
                    </div>
                  )}
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: "5px 0",
                        fontSize: 13,
                        color: isChosen ? "var(--good)" : "inherit",
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    {responses.map((r) => (
                      ...
                      >
                        ...
                        >
                          <span>
                            {r.mc_agents?.avatar_emoji || "\u{1F916}"}
                          </span>
                        ...
                        {r.risk_flags && r.risk_flags.length > 0 && (
                          ...
                          >
                            {r.risk_flags.map((rf, j) => (
                              <span
                                key={j}
                                className={`badge ${severityBadge(rf.severity)}`}
                                style={{ fontSize: 10 }}
                              >
                                {rf.risk}
                              </span>
              ...
              {/* Decision action for open boards */}
              {board.status === "open" && (
                <div
                  style={{
                    borderTop: "1px solid var(--line)",
                  ...
                  {decidingBoard === board.id ? (
                    <div style={{ display: "grid", gap: 8 }}>
                        ...
                        {board.options.map((opt) => (
                          <button
                            ...
                            style={{
                              ...
                              fontSize: 12,
                              fontWeight:
                                selectedOption === opt.label ? 600 : 400,
                            }}
                          >
                            {opt.label}: {opt.summary.slice(0, 60)}
                            {opt.recommended_by &&
                              opt.recommended_by.length > 0 && (
                                <span
                                  className="muted"
                                  style={{ marginLeft: 6 }}
                                >
                                  ({opt.recommended_by.length} votes)
                                </span>
                              )}
                          </button>
                        ))}
                      ...
                      <textarea
                        ...
                        placeholder="Rationale (optional) — why this option?"
                        rows={2}
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <button
                          ...
                          onClick={() => recordDecision(board.id)}
                    ...
                    <button
                      className="btn-sm btn-primary"
                      onClick={() => setDecidingBoard(board.id)}
                    >
                      Make Decision
                    </button>
                  )}
              ...
              {board.status === "decided" && (
                ...
                >
                  <div style={{ fontSize: 13 }}>
                    ...
                    {board.rationale && (
                      <span className="muted"> &mdash; {board.rationale}</span>
                    )}
                  </div>
                  {board.decided_at && (
                    <div
          ...
          <div className="card" style={{ textAlign: "center", padding: 40 }}>
            ...
            <div style={{ fontWeight: 600, marginBottom: 4 }}>
              {filter === "open"
                ...
                : filter === "decided"
                  ? "No decisions yet"
                  : "No challenge boards yet"}
            </div>
            <div className="muted">
              Ask Ed to create one: &quot;Should we launch MyMeme for
              Mother&apos;s Day?&quot;
            </div>
          </div>
        )}
