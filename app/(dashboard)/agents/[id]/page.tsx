...
export default function AgentDetailPage() {
  ...
  async function toggleSkill(skillId: string, assigned: boolean) {
    ...
    const res = await fetch(`/api/agents/${params.id}/skills`, {
      cache: "no-store",
    });
  ...
  return (
    <div>
      <Link href="/agents" style={{ fontSize: 13 }}>
        &larr; Back to Agents
      </Link>
      ...
      <p className="page-sub">
        {agent.role} &bull; {agent.mc_departments?.name || "No department"}{" "}
        &bull; {agent.default_engine}
        {agent.model_id ? ` (${agent.model_id})` : ""}
        {agent.reports_to_name
          ? ` &bull; reports to ${agent.reports_to_name}`
          : ""}
      </p>
      ...
      <section className="grid" style={{ marginBottom: 14 }}>
        ...
        <article className="card card-glow" style={{ gridColumn: "span 3" }}>
          ...
          <div
            className="kpi"
            style={{
              color: agent.consecutive_failures > 0 ? "var(--bad)" : undefined,
            }}
          >
      ...
      <section className="grid" style={{ marginBottom: 14 }}>
        ...
        <article className="card" style={{ gridColumn: "span 6" }}>
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            {skills.length === 0 && (
              <div className="muted">No skills available</div>
            )}
          </div>
        </article>
      </section>
                ...
                <tbody>
                  {prompts.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>
                        {p.performance_delta !== null
                          ? `${p.performance_delta > 0 ? "+" : ""}${p.performance_delta}`
                          : "—"}
                      </td>
              ...
              <tbody>
                {reviews.map((r) => (
                  <tr key={r.id}>
                    <td>
                      <span
                        className={`badge ${r.total_score >= 35 ? "good" : "bad"}`}
                      >
                        {r.total_score}/50
                      </span>
                    </td>
