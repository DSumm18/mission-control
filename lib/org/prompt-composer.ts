...
export async function composePrompt(
  ...
  const { data: job } = await sb
    .from("mc_jobs")
    .select(
      "id, title, prompt_text, command, project_id, review_notes, parent_job_id",
    )
    .eq("id", jobId)
    .single();
  ...
  if (job.project_id) {
    ...
    if (project) {
      ...
      if (project.revenue_target_monthly) {
        parts.push(
          `**Revenue Target:** £${project.revenue_target_monthly}/month`,
        );
      }
      if (
        project.delivery_plan &&
        Object.keys(project.delivery_plan).length > 0
      ) {
      ...
      if (deliverables?.length) {
        ...
        const planning = deliverables.filter((d) =>
          planningTypes.includes(d.deliverable_type),
        );
        const approved = planning.filter((d) => d.status === "approved").length;
        if (planning.length > 0) {
          parts.push(
            `**Planning gate:** ${approved}/${planning.length} approved`,
          );
        }
        for (const d of deliverables) {
          parts.push("");
          parts.push(
            `### ${d.deliverable_type.toUpperCase()}: ${d.title} [${d.status}]`,
          );
  ...
  if (job.review_notes) {
    ...
    parts.push("");
    parts.push(
      "Address the above feedback in this attempt. Improve on the areas flagged.",
    );
  }
  ...
  parts.push(
    "- **Web Search** — search the internet for current information, news, data, government publications. USE THIS for any research task.",
  );
  parts.push(
    "- **Web Fetch** — fetch and read the contents of any URL (articles, gov.uk pages, PDFs, documentation).",
  );
  parts.push(
    "- **Bash** — run shell commands, scripts, build tools, git operations.",
  );
  parts.push(
    "- **File Read/Write/Edit** — read, create, and modify files in the project repo.",
  );
  parts.push(
  ...
  parts.push("");
  parts.push(
    "When researching topics, ALWAYS use Web Search to find current data and sources. Do not rely solely on your training data.",
  );
  ...
  if (agentSkills && agentSkills.length > 0) {
    ...
    for (const as of agentSkills) {
      ...
      if (skill) {
        parts.push(
          `- **${skill.key}**${skill.mcp_server_name ? ` (MCP: ${skill.mcp_server_name})` : ""}: ${skill.usage_guidelines || "No guidelines set"}`,
        );
      }
    }
  }

  // 7. Prompt skills (domain knowledge from workspace SKILL.md files)
  const { data: promptSkills } = await sb
    .from("mc_agent_skills")
    .select(
      "skill_id, mc_skills(key, usage_guidelines, skill_type, prompt_content, prompt_file_path)",
    )
    .eq("agent_id", agentId)
    .eq("allowed", true);

  const promptEntries: { key: string; content: string }[] = [];
  if (promptSkills) {
    for (const ps of promptSkills) {
      const skill = ps.mc_skills as unknown as {
        key: string;
        usage_guidelines: string | null;
        skill_type: string;
      ...
      if (!skill || skill.skill_type !== "prompt") continue;

      let content = skill.prompt_content;
      if (!content && skill.prompt_file_path) {
        try {
          content = await fs.readFile(skill.prompt_file_path, "utf8");
        } catch {
          // File not found — skip this skill
          continue;
        }
      }
      if (!content) continue;
      ...
      const truncated =
        content.length > MAX_SKILL_CHARS
          ...
          : content;
      promptEntries.push({ key: skill.key, content: truncated });
    }
  }

  if (promptEntries.length > 0) {
    parts.push("");
    parts.push("## Domain Knowledge (prompt skills)");
